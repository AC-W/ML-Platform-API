...
def load_car_model():
    ...
    if (car_net):
        ...
        state = torch.hub.load_state_dict_from_url("https://github.com/AC-W/ML-Platform-API/raw/main/Models/CarRecognitionModel",map_location=torch.device('cpu'))
...
async def predict_car_image(file: UploadFile = File(...),url : str = File()):
    global car_net
    if (car_net):
...
@app.post('/predict_car_model_url')
async def predict_car_image(url: str = File()):
    global car_net
    if (car_net):
